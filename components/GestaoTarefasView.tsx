...
// ─────────────────────────────────────────────────────────────────────────────
const GestaoTarefasView: React.FC<GestaoTarefasViewProps> = ({ tasks, contracts, onUpdateTaskStatus, onUpdateTask, onNavigateToDetails, onSimulateRenegotiation }) => {
  const [taskAtiva, setTaskAtiva] = useState<Task | null>(null);
  const [resumoNegociacao, setResumoNegociacao] = useState("");

  // Carregar resumo quando a task ativa mudar
  React.useEffect(() => {
    if (taskAtiva) {
      setResumoNegociacao(taskAtiva.description || "");
    } else {
      setResumoNegociacao("");
    }
  }, [taskAtiva]);

  // Histórico de Propostas do Contrato
  const historicoPropostas = useMemo(() => {
    return taskAtiva?.negotiationHistory || [];
  }, [taskAtiva]);
  ...
  const exportarRelatorio = () => {
    ...
    const rows = sortedTasks.map(t => {
      ...
      const historicoStr = historicoPropostas.map(h => `${h.date}: ${h.value} (${h.details})`).join(' | ');
          ...
          {sortedTasks.map((t, index) => {
            ...
            return (
              <div key={`${t.id}-${index}`} 
                onClick={() => {
                  if (contract) {
                    onSimulateRenegotiation(contract.saldoDevedor, contract.daysOverdue, contract.id);
                  }
                }}
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        onUpdateTaskStatus(t.id, t.status === TaskStatus.Concluida ? TaskStatus.Pendente : TaskStatus.Concluida);
                      }}
                      className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${t.status === TaskStatus.Concluida ? 'bg-[var(--status-success)] border-[var(--status-success)] text-white' : 'border-[var(--border-default)] bg-[var(--surface-background)] hover:border-[var(--brand-primary)]'}`}
                      title={t.status === TaskStatus.Concluida ? "Marcar como Pendente" : "Marcar como Concluída"}
                    >
                      {t.status === TaskStatus.Concluida && <FeatherIcon name="check" className="w-4 h-4" />}
                    </button>
                ...
                <div className={t.status === TaskStatus.Concluida ? 'opacity-50' : ''}>
                  <p className={`font-black text-[var(--text-primary)] mt-2 uppercase italic ${t.status === TaskStatus.Concluida ? 'line-through' : ''}`}>{t.contractClient}</p>
                  <p className="text-[10px] text-[var(--text-secondary)] mt-1 line-clamp-1">{t.description}</p>
                </div>
                
                <div className="mt-4 flex justify-between items-center">
                  <span className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">{t.status}</span>
                  <div className="flex gap-2">
                    <button 
                      onClick={(e) => { e.stopPropagation(); setTaskAtiva(t); }}
        ...
        <AnimatePresence mode="wait">
          {taskAtiva ? (
            <motion.div key={taskAtiva.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              className="box-glow rounded-[3rem] shadow-2xl overflow-hidden">
              ...
              <div className="p-8 space-y-8">
                  ...
                  <div className="space-y-2">
                    {historicoPropostas.map((h, i) => (
                      <div key={i} className="flex justify-between p-4 bg-[var(--surface-background)] rounded-2xl border border-[var(--border-default)]">
                        <div>
                          <p className="text-xs font-black text-[var(--text-primary)]">{h.value}</p>
                          <p className="text-[10px] text-[var(--brand-primary)] font-bold uppercase italic">{h.details}</p>
                        </div>
                        <span className="text-[10px] text-[var(--text-secondary)] font-mono">{h.date}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  <button 
                    onClick={() => {
                      if (taskAtiva) {
                        onUpdateTask(taskAtiva.id, { description: resumoNegociacao });
                        alert("Interação salva com sucesso no dossiê.");
                      }
                    }}
                    className="flex-1 py-5 bg-[var(--brand-primary)] hover:bg-[var(--brand-primary)]/90 text-white font-black rounded-2xl text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-blue-600/20 transition-all hover:scale-[1.02]"
                  >
                    Salvar Resumo e Histórico
                  </button>
                  <button 
                    onClick={() => {
                      const contract = contracts.find(c => c.id === taskAtiva.contractId);
                      if (contract) {
                        onSimulateRenegotiation(contract.saldoDevedor, contract.daysOverdue, contract.id);
                      }
                    }}
                    className="px-8 py-5 bg-[var(--status-success)] hover:bg-[var(--status-success)]/90 text-white font-black rounded-2xl text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-emerald-500/20 transition-all hover:scale-[1.02]"
                  >
                    Converter em Acordo
                  </button>
                </div>
