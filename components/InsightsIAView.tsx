    ...
    <div className="space-y-10 animate-fade-in max-w-[1400px] mx-auto pb-20">
      <div className="bg-slate-900 text-white p-12 rounded-[4rem] shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm font-bold text-[#a0aec0] uppercase tracking-[0.4em]">Varredura Tripla: Risco, Volume e Urgência</p>
          <div className="flex flex-wrap gap-4 mt-10">
            ...
            <button 
              ...
              className="px-10 py-4 bg-[#242938] border border-[#2e3347] rounded-3xl text-[10px] font-black uppercase tracking-widest hover:bg-[#2e3347] transition-all relative"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Top 10 Ofensores */}
        <div className="bg-[#1a1f2e] p-10 rounded-[3rem] border border-[#2e3347] shadow-xl">
          <div className="flex items-center justify-between mb-8">
            <h4 className="text-xl font-black uppercase italic text-[#f0f4ff]">Top 10 Ofensores</h4>
            <FeatherIcon name="alert-octagon" className="w-8 h-8 text-rose-500" />
          </div>
          <div className="space-y-4">
            {topOfensores.map((c, idx) => (
              <div key={c.id} className="flex flex-col p-4 bg-[#242938]/50 rounded-2xl hover:bg-[#242938] transition-colors cursor-pointer" onClick={() => onNavigateToDetails(c.id)}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black text-[#a0aec0]">#{idx + 1}</span>
                    <p className="text-xs font-black text-[#f0f4ff] uppercase truncate max-w-[200px]">⚠️ ALERTA: {c.clientName}</p>
                  </div>
                  <p className="text-sm font-black text-rose-600 tabular-nums">{formatCurrency(c.saldoDevedor)}</p>
                </div>
                <div className="flex items-center justify-between text-[10px] font-bold text-[#a0aec0] uppercase tracking-widest">
        ...
        <div className="bg-[#1a1f2e] p-10 rounded-[3rem] border border-[#2e3347] shadow-xl">
          <div className="flex items-center justify-between mb-8">
            <h4 className="text-xl font-black uppercase italic text-[#f0f4ff]">Diretrizes de Performance</h4>
            <FeatherIcon name="target" className="w-8 h-8 text-blue-500" />
          </div>
          <div className="grid grid-cols-1 gap-4">
            {diretrizes.map((d) => (
              <div key={d.faixa} className="p-6 bg-[#242938]/50 rounded-2xl border border-transparent hover:border-blue-500/30 transition-all">
                ...
                <p className="text-sm font-bold text-[#cbd5e0]">{d.acao}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-[#1a1f2e] p-10 rounded-[3rem] border border-[#2e3347] shadow-xl">
           ...
           <h4 className="text-lg font-black uppercase italic text-[#f0f4ff] mb-2">Anomalias de Risco</h4>
           <p className="text-xs font-bold text-[#a0aec0] leading-relaxed uppercase">Sócios com saltos súbitos de faixa de atraso são priorizados no feed.</p>
        </div>
        <div className="bg-[#1a1f2e] p-10 rounded-[3rem] border border-[#2e3347] shadow-xl">
           <FeatherIcon name="activity" className="w-10 h-10 text-blue-500 mb-6" />
           <h4 className="text-lg font-black uppercase italic text-[#f0f4ff] mb-2">Desvio de Volume</h4>
           <p className="text-xs font-bold text-[#a0aec0] leading-relaxed uppercase">Análise de concentração de saldo por gerência comparada à média global.</p>
        </div>
        <div className="bg-[#1a1f2e] p-10 rounded-[3rem] border border-[#2e3347] shadow-xl">
           <FeatherIcon name="zap" className="w-10 h-10 text-amber-500 mb-6" />
           <h4 className="text-lg font-black uppercase italic text-[#f0f4ff] mb-2">Ações de Urgência</h4>
           <p className="text-xs font-bold text-[#a0aec0] leading-relaxed uppercase">Contratos "High Value" com canais de contato ativos identificados.</p>
        </div>
      </div>

      <div className="bg-[#1a1f2e] p-12 rounded-[4rem] border border-[#2e3347] shadow-2xl max-w-none">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center space-y-6">
            <div className="w-16 h-16 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
            <p className="text-[10px] font-black uppercase tracking-widest text-[#a0aec0]">A IA está processando os dados da Cooperativa...</p>
          </div>
        ) : (
          <div className="text-[#cbd5e0] font-medium prose dark:prose-invert max-w-none">
             {aiAnalysis ? (
               <ReactMarkdown>{aiAnalysis}</ReactMarkdown>
             ) : (
               <p className="text-center text-[#a0aec0] uppercase tracking-widest text-xs py-10">Inicie a varredura para ver os insights do Motor de IA.</p>
             )}
          </div>
        )}
      </div>
