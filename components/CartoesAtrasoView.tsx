...
const CartoesAtrasoView: React.FC<CartoesAtrasoViewProps> = ({ contracts, onNavigateToDetails, onContractClick }) => {
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-[var(--border-default)]/50">
                      {cardContracts.map((c, index) => (
                        <tr key={`${c.id}-${index}`} className="hover:bg-[var(--status-error)]/5 transition-all group cursor-pointer" onClick={() => onContractClick ? onContractClick(c) : onNavigateToDetails(c.id)}>
