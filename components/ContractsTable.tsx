...
const ContractsTable: React.FC<ContractsTableProps> = ({ contracts, initialSearchTerm = '', onNavigateToDetails, onSimulateRenegotiation, userRole = UserRole.Gerente }) => {
  ...
  return (
    <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 box-glow p-6 rounded-[2.5rem]">
            <div className="relative flex-grow max-w-md">
                <input
                    type="text"
                    placeholder="Buscar Sócio, CPF ou Contrato..."
                    value={searchTerm}
                    className="w-full bg-[var(--surface-background)] border-2 border-[var(--border-default)] rounded-2xl px-6 py-3.5 text-sm font-black outline-none focus:border-[var(--brand-primary)] transition-all text-[var(--text-primary)]"
      ...
      <div className="overflow-x-auto rounded-[3rem] box-glow shadow-2xl">
            ...
            {sortedContracts.map((c, index) => (
              <tr key={`${c.id}-${index}`} className="hover:bg-[var(--brand-primary)]/5 transition-colors group">
                ...
                <td 
                    ...
                    onClick={() => onSimulateRenegotiation && onSimulateRenegotiation(c.saldoDevedor, c.daysOverdue, c.id)}
