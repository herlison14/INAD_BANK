...
const PredictiveAIAlerts: React.FC<PredictiveAIAlertsProps> = ({ contracts, onNavigateToDetails }) => {
  ...
  return (
    <div className="mb-12 bg-[#1a1f2e] p-[1px] rounded-[3.5rem] overflow-hidden shadow-2xl">
      <div className="bg-[#1a1f2e] p-10 rounded-[3.5rem]">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-10 gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-[#f0f4ff] uppercase tracking-tighter italic leading-none">Monitor Preditivo Gemini AI</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <AnimatePresence mode="wait">
            {loading ? (
              [1, 2, 3].map(i => (
                <div key={i} className="h-48 bg-[#242938] animate-pulse rounded-[2.5rem] border border-[#2e3347]" />
              ))
            ) : error ? (
              <div className="col-span-full py-12 text-center text-[#a0aec0] font-bold uppercase text-xs tracking-widest bg-[#242938] rounded-[2.5rem] border border-dashed border-[#2e3347]">
                {error}
              </div>
            ) : insights.length > 0 ? (
              insights.map((alert, idx) => (
                <motion.div 
                  ...
                  className="p-8 bg-[#242938] border border-[#2e3347] rounded-[2.5rem] hover:bg-[#2e3347]/80 transition-all group cursor-pointer relative overflow-hidden"
                >
                  ...
                  <div className="flex justify-between items-start mb-6 relative z-10">
                    ...
                    <FeatherIcon name="alert-circle" className="text-[#f0f4ff]/20 group-hover:text-blue-400 transition-colors" />
                  </div>
                  
                  <h4 className="font-black text-[#f0f4ff] text-lg mb-2 truncate italic">{alert.client}</h4>
                  <p className="text-[11px] text-[#a0aec0] leading-relaxed mb-6 h-12 overflow-hidden line-clamp-3 font-medium uppercase tracking-tight">
                    {alert.reason}
                  </p>
                  
                  <div className="pt-6 border-t border-[#2e3347] flex flex-col gap-2 relative z-10">
                    <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest">Ação Sugerida:</span>
                    <span className="text-[11px] font-black text-[#f0f4ff] leading-tight uppercase italic">{alert.action}</span>
                  </div>
                </motion.div>
              ))
            ) : (
              <div className="col-span-full py-12 text-center text-[#a0aec0] font-bold uppercase text-xs tracking-widest border border-dashed border-[#2e3347] rounded-[2.5rem]">
