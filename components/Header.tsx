...
const Header: React.FC<HeaderProps> = ({ onMenuClick, darkMode, toggleDarkMode, currentUser, searchValue, onSearchChange, onDataImported }) => {
  ...
  return (
    <header className="bg-[var(--surface-container)]/80 backdrop-blur-xl border-b border-[var(--border-default)] p-4 px-6 flex flex-col md:flex-row justify-between items-center sticky top-0 z-40 gap-4 transition-all">
      <div className="flex items-center flex-1 w-full gap-4">
        <button onClick={onMenuClick} className="text-[var(--text-secondary)] lg:hidden hover:bg-[var(--surface-background)] p-2 rounded-xl transition-colors">
          <FeatherIcon name="list" />
        </button>
        
        <div className="hidden sm:flex items-center mr-4">
          <div className="w-10 h-10 bg-[var(--brand-primary)] rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 mr-3">
            <span className="text-white font-black text-xl italic">P</span>
          </div>
          <h1 className="text-xl font-extrabold tracking-tighter text-[var(--text-primary)] italic leading-none">
            PAINEL <span className="text-[var(--brand-primary)]">INAD 1.0</span>
          </h1>
        </div>

        {/* Global Search Bar */}
        <div className="relative w-full max-w-sm group">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-[var(--text-secondary)] group-focus-within:text-[var(--brand-primary)] transition-colors">
          ...
          <input 
            ...
            className="w-full bg-[var(--surface-background)] border border-[var(--border-default)] rounded-2xl py-2.5 pl-11 pr-4 text-sm font-medium focus:ring-2 focus:ring-[var(--brand-primary)] text-[var(--text-primary)] transition-all placeholder:text-[var(--text-secondary)]"
