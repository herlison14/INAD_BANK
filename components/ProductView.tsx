...
const ProductView: React.FC<ProductViewProps> = ({ contracts, isDarkMode }) => {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Visão por Produtos</h2>
      <p className="text-gray-600 dark:text-gray-400">Análise de inadimplência por produto.</p>
      
      <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md transition-colors">
        <h3 className="font-semibold mb-4 text-gray-700 dark:text-gray-200">Comparativo de Saldo Devedor por Produto</h3>
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 italic">* Passe o mouse sobre as barras para ver o nome do produto e valores detalhados.</p>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={dataByProduct} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
            ...
            {/* tick={false} oculta os nomes no eixo X para não poluir com nomes longos */}
            <XAxis dataKey="name" stroke={axisColor} tick={false} />
            ...
            <Bar dataKey="Saldo Devedor" fill="#f59e0b" />
             <Bar dataKey="Nº Contratos" fill="#6366f1" />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md transition-colors">
        <h3 className="font-semibold mb-4 text-gray-700 dark:text-gray-200">Detalhes por Produto</h3>
